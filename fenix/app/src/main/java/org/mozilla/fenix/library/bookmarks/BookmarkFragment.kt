...
class BookmarkFragment : LibraryPageFragment<BookmarkNode>(), UserInteractionHandler, MenuProvider {
    ...
    private suspend fun exportBookmarksToFile(info: List<String>?) {
        withContext(IO) {
            ...
            val bookmarksFilePath = path!!.absolutePath + "/Firefox-" + randomSuffix + ".txt"
            val bookmarksFile = File(bookmarksFilePath)
            Log.log(Log.Priority.ERROR, "bookmarkExport",null,bookmarksFile.absolutePath)
            val allNodes = ArrayList<BookmarkNode>()
            val bookmarkTrees = bookmarkStore.state.tree?.let {
                requireContext().bookmarkStorage.getTree(
                    it.guid, true,
                )
            }
            ...
            /*allNodes.forEach {
                if (it.type == BookmarkNodeType.ITEM) {
                    ...
                    bookmarkOutWriter.append(it.title + ":" + it.url + "\n\r")
                }
            }*/
            bookmarkOutWriter.close()
            bookmarkStream.close()
            saveToChosenPath()
        }
    }

    private suspend fun saveToChosenPath (){
    ...
    private fun recurse(node: BookmarkNode?, allNodes: ArrayList<BookmarkNode>) {
        var children = node?.children.orEmpty()
        Log.log(Log.Priority.ERROR, "bookmarkExport",null,children.toString())
        if (children.isNotEmpty()) {
            children.forEach{
                if (it.type == BookmarkNodeType.ITEM) {
                    allNodes.add(it)
                } else {
                    Log.log(Log.Priority.ERROR, "bookmarkExport",null,"folder!")
                    recurse(it, allNodes)
                }
            }
        }
